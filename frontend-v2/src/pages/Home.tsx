				...
				<Fade in timeout={800}>
					...
					>
						<CardContent>
							...
							<Button component={Link} to="/trip/new" variant="contained" size="large" startIcon={<Add />}>
				...
				{ownedTrips.map((t, index) => (
					<Grid item xs={12} sm={6} md={4} key={t.id}>
						<Fade in timeout={500 + index * 100}>
							...
							>
								<CardActionArea component={Link} to={`/trips/${t.id}`} sx={{ height: '100%' }}>
									<CardContent sx={{ p: 3 }}>
										...
										<Stack direction="row" spacing={1} sx={{ mt: 2 }} flexWrap="wrap" gap={1}>
											{t.flights?.length > 0 && (
												<Chip size="small" icon={<Flight sx={{ fontSize: 16 }} />} label={`${t.flights.length}`} variant="outlined" color="primary" />
											)}
											{t.hotels?.length > 0 && (
												<Chip size="small" icon={<Hotel sx={{ fontSize: 16 }} />} label={`${t.hotels.length}`} variant="outlined" color="secondary" />
											)}
				...
				<Box sx={{ mt: 6 }}>
					<Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
						<Box display="flex" alignItems="center" gap={2}>
							...
							<Chip icon={<VisibilityIcon />} label="View Only" size="small" color="info" variant="outlined" />
						</Box>
						<Typography variant="body2" color="text.secondary">
										...
										<CardActionArea component={Link} to={`/trips/${t.id}`} sx={{ height: '100%' }}>
											<CardContent sx={{ p: 3 }}>
												<Stack direction="row" justifyContent="space-between" alignItems="start" mb={2}>
													...
													<Chip icon={<VisibilityIcon sx={{ fontSize: 16 }} />} label="View Only" size="small" color="info" sx={{ ml: 1 }} />
												</Stack>
